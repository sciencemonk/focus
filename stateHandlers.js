...
var stateHandlers = {
    startModeIntentHandlers : Alexa.CreateStateHandler(constants.states.START_MODE, {
        ...
        'Unhandled' : function () {
            var message = 'Sorry, I could not understand. Please say, go or begin to start.';
            this.response.speak(message).listen(message);
            this.emit(':responseReady');
        }
    }),
    playModeIntentHandlers : Alexa.CreateStateHandler(constants.states.PLAY_MODE, {
        ...
        'LaunchRequest' : function () {
            ...
            if (this.attributes['playbackFinished']) {
                ...
                message = 'You were on ' + audioData[this.attributes['playOrder'][this.attributes['index']]].title +
                    ' Would you like to resume?';
                reprompt = 'You can say yes to resume or no to start over';
            }
        ...
        'Unhandled' : function () {
            var message = 'Sorry, I could not understand. You can say, Next or Previous to navigate through each level.';
            this.response.speak(message).listen(message);
            this.emit(':responseReady');
        }
    }),
    ...
    resumeDecisionModeIntentHandlers : Alexa.CreateStateHandler(constants.states.RESUME_DECISION_MODE, {
         ...
         */
        'LaunchRequest' : function () {
            var message = 'You were on ' + audioData[this.attributes['playOrder'][this.attributes['index']]].title +
                ' Would you like to resume?';
            var reprompt = 'You can say yes to resume or no to start over.';
            this.response.speak(message).listen(reprompt);
            this.emit(':responseReady');
        },
        ...
        'AMAZON.HelpIntent' : function () {
            var message = 'You were on ' + audioData[this.attributes['index']].title +
                ' Would you like to resume?';
            var reprompt = 'You can say yes to resume or no to start over.';
            this.response.speak(message).listen(reprompt);
            this.emit(':responseReady');
        ...
        playPrevious: function () {
            ...
            if (index === -1) {
                if (this.attributes['loop']) {
                    ...
                    var message = 'You have reached the beginning';
                    this.response.speak(message).audioPlayerStop();
                    return this.emit(':responseReady');
                }
            }
